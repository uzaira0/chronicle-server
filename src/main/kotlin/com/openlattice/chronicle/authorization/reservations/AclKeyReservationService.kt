...
public open class AclKeyReservationService(private val resolver: StorageResolver) {
    ...
    // reason: nesting is the connection.use/prepareStatement.use/executeQuery.use JDBC resource idiom
    ...
    public fun getIds(names: Set<String>): Map<String, UUID> {
    ...
    public fun renameReservation(oldName: String, newName: String) {
        resolver.getPlatformStorage().connection.use { conn ->
    ...
    public fun renameReservation(id: UUID, newName: String) {
        resolver.getPlatformStorage().connection.use { conn ->
    ...
    public fun <T : AbstractSecurableObject> registerSecurableObject(
    ...
    public fun <T : AbstractSecurableObject> registerSecurableObject(
        ...
        //Future: Back off 50ms each attempt for a maximum of 10 attempts, waiting no more than a second.
        repeat(10) {
            resolver.getPlatformStorage().connection.use { conn ->
                val aclKey = AclKey(prefix + obj.id)
                conn.prepareStatement(INSERT_SQL).use { ps ->
                    val assignedId = tryAssignSecurableObjectId(ps, conn, aclKey, obj, proposedName)
                    if (assignedId != null) {
                        return assignedId
                    }
        ...
        error("Unable to assign an id for object ${obj.title} of type ${obj.category}")
    }

    private fun <T : AbstractSecurableObject> tryAssignSecurableObjectId(
     ...
     * @param id The id to release.
     */
    public fun releaseByIds(ids: Collection<UUID>) {
        resolver.getPlatformStorage().connection.use { conn ->
     ...
     */
    public fun releaseByNames(names: Collection<String>) {
        resolver.getPlatformStorage().connection.use { conn ->
