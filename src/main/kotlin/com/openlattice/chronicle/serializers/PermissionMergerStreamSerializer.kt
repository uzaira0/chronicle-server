 ...
 */
@Component
public open class PermissionMergerStreamSerializer : SelfRegisteringStreamSerializer<PermissionMerger?> {
    ...
    internal companion object {
        ...
        public fun deserialize(input: ObjectDataInput): EnumSet<Permission> {
            val bs = BitSet.valueOf(input.readLongArray()!!)
        ...
        @Throws(IOException::class)
        public fun serialize(out: ObjectDataOutput, permissions: Iterable<Permission>) {
            val bs = BitSet(P.size)
            for (p in permissions) {
                bs.set(p.ordinal)
            }
            out.writeLongArray(bs.toLongArray())
            // Note: Move this method to a class where it is not as hidden
        }
    }
}
