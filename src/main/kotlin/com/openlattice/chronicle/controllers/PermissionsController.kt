...
@SuppressFBWarnings(
    ...
    justification = "https://github.com/spotbugs/spotbugs/issues/927"
)
public open class PermissionsController @Inject constructor(
    public val securePrincipalsManager: SecurePrincipalsManager,
    ...
    override fun updateAcl(@Valid @RequestBody req: AclData): OK {
        auditService.logWithContext {
            action(AuditAction.PERMISSION_CHANGE)
            resourceType("Permission")
            resourceId(req.acl.aclKey.first())
            success(true)
            additionalData(mapOf("action" to req.action.name))
        }
    ...
    // reason: boundary catch — any ACL-mutation failure type is audited as failed then rethrown so the audit trail records the attempt
    @Suppress("TooGenericExceptionCaught")
    override fun updateAcls(@Valid @RequestBody @Size(max = 10_000) req: List<AclData>): OK {
        ...
        try {
            ...
            auditService.logWithContext {
                ...
                resourceType("Permission")
                success(true)
                additionalData(mapOf("aclCount" to req.size))
            }
        } catch (ex: Exception) {
            auditService.logWithContext {
                action(AuditAction.PERMISSION_CHANGE)
                resourceType("Permission")
                failed(ex.message ?: "ACL update failed")
            }
            throw ex
        }
        return ok
    ...
    override fun getAcl(@Valid @RequestBody aclKey: AclKey): Acl {
        if (!isAuthorized(Permission.OWNER).test(aclKey)) {
            throw ForbiddenException(
                "Only owner of securable object " + aclKey +
                        " can access other users' access rights."
            )
        }
        val acl = authorizationManager.getAllSecurableObjectPermissions(aclKey)
        auditService.logWithContext {
            action(AuditAction.VIEW)
            resourceType("Permission")
            resourceId(aclKey.first())
            success(true)
        }
        return acl
    }
    ...
    override fun getAcls(@Valid @RequestBody aclKeys: Set<AclKey>): Set<Acl> {
        ensureOwnerAccess(aclKeys)
        val acls = authorizationManager.getAllSecurableObjectPermissions(aclKeys)
        auditService.logWithContext {
            action(AuditAction.VIEW)
            resourceType("Permission")
            success(true)
            additionalData(mapOf("aclKeyCount" to aclKeys.size))
        }
        return acls
    }
    ...
    // reason: authorization graph traversal building principal grant paths; the nested
    // layer/parent/path loops share mutable accumulators and must not be split lest the
    // permission-explanation result change
    @Suppress("LongMethod", "NestedBlockDepth")
    override fun getAclExplanation(@Valid @RequestBody aclKey: AclKey): Collection<AclExplanation> {
        ...
        auditService.logWithContext {
            ...
            resourceId(aclKey.first())
            success(true)
            additionalData(mapOf("endpoint" to "getAclExplanation"))
        }
