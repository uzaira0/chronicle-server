...
import com.openlattice.chronicle.timeusediary.TimeUseDiaryColumnTitles
...
import com.openlattice.chronicle.timeusediary.TimeUseDiarySummarizedEntity
 ...
 */
public open class TimeUseDiaryService(
    ...
    override fun submitTimeUseDiary(
        ...
        require(responses.size <= 1_000) { "Time use diary submission too large: ${responses.size} entries (max 1,000)" }
        ...
        logger.info(
            "submitted time use diary responses studyId={} participant={}",
            studyId,
    ...
    override fun getParticipantTUDSubmissionsByDate(
        ...
        try {
            ...
            hds.connection.use { connection ->
                val result = executeGetSubmissionByDateSql(
                    connection,
                    studyId,
                    participantId,
                    startDate,
                ...
                collectSubmissionsByDate(result, submissionsByDate)
            }
        } catch (ex: SQLException) {
            logger.error(
                "Error fetching participant TUD submissions by date for studyId={}, participantRef={}",
                studyId,
                LogSanitizer.stableFingerprint(participantId, "participant"),
                ex
            )
    ...
    private fun collectSubmissionsByDate(
        result: ResultSet,
        submissionsByDate: MutableMap<OffsetDateTime, MutableSet<UUID>>,
    ) {
        while (result.next()) {
    ...
    override fun getStudyTUDSubmissionIdsByDate(
        ...
        try {
            ...
            logger.error("Error fetching submission ids by date for studyId={}", studyId, ex)
            throw ex
        }
    }

    // reason: boundary catch — must log and rethrow any failure type from the DB read/JSON-decode path
    ...
    override fun getStudyTUDSubmissions(
        ...
        try {
            ...
            logger.error("Error downloading TUD data for studyId={}", studyId, ex)
            throw ex
        }
    }
    ...
    private fun getTimeUseDiarySummarizedData(
        ...
        try {
            ...
            return TimeUseDiaryPostgresDownloadWrapper(iterable)
                .withColumnAdvice(TimeUseDiaryDownloadDataType.Summarized.downloadColumnTitles.toList())
        } catch (ex: Exception) {
            logger.error("Error downloading summarized TUD data for studyId={}", studyId, ex)
            throw ex
        }
    }
    ...
    private fun getDefaultColumnMapping(rs: ResultSet): Map<String, Any> {
        return mapOf(
            TimeUseDiaryColumnTitles.PARTICIPANT_ID to rs.getString(PARTICIPANT_ID.name),
            TimeUseDiaryColumnTitles.TIMESTAMP to rs.getObject(SUBMISSION_DATE.name, OffsetDateTime::class.java),
            TimeUseDiaryColumnTitles.STUDY_ID to rs.getObject(STUDY_ID.name),
            TimeUseDiaryColumnTitles.SUBMISSION_ID to rs.getObject(SUBMISSION_ID.name)
        )
    }
    ...
    private fun getDayTimeDataColumnMapping(rs: ResultSet): List<Map<String, Any>> {
        ...
        responsesByStartDateTime.forEach { (startDateTime, responses) ->
            ...
            val mappedColumns = mapOf(
                TimeUseDiaryColumnTitles.ACTIVITY_DURATION to ChronoUnit.MINUTES.between(startDateTime, endDateTime),
                TimeUseDiaryColumnTitles.ACTIVITY_COUNTER to ++counter,
                TimeUseDiaryColumnTitles.ACTIVITY_START_TIME to startDateTime!!.toLocalTime().format(formatter),
                TimeUseDiaryColumnTitles.ACTIVITY_END_TIME to endDateTime!!.toLocalTime().format(formatter)
            ...
            val timeRangeColumnMapping = additionalColumTitles.associateWith { title ->
                val questionCode = TimeUseDiaryColumnTitles.columnTitleToQuestionCodeMap[title] ?: return@associateWith "UNMAPPED"
            ...
            val nonTimeRangeColumnMapping = unmappedColumnTitles.associateWith { title ->
                val questionCode = TimeUseDiaryColumnTitles.columnTitleToQuestionCodeMap.getValue(title)
    ...
    // reason: TUD night-time mapping preserves a deliberate legacy-compatibility bug +
    // interdependent time arithmetic; restructuring risks altering exported data
    ...
    private fun getNightTimeDataColumnMapping(rs: ResultSet): List<Map<String, Any>> {
        ...
        val sleepHoursMapping = if (todayWakeUpDateTime != null) {
            mapOf(
                TimeUseDiaryColumnTitles.WAKE_UP_TODAY to (todayWakeUpDateTime.toLocalTime()?.format(formatter) ?: ""),
                TimeUseDiaryColumnTitles.SLEEP_HOURS to ChronoUnit.HOURS.between(
            ...
            if (bedTimeBeforeActivityDayDateTime != null) {
                mapOf(
                    TimeUseDiaryColumnTitles.BEDTIME_AFTER_ACTIVITY_DAY to (bedTimeBeforeActivityDayDateTime.toLocalTime()
                        ?.format(formatter) ?: ""),
                    TimeUseDiaryColumnTitles.SLEEP_HOURS to ChronoUnit.HOURS.between(
                ...
                mapOf(
                    TimeUseDiaryColumnTitles.WAKE_UP_TIME_AFTER_ACTIVITY_DAY to (wakeUpTimeAfterActivityDayDateTime.toLocalTime()
                        .format(formatter)),
                    TimeUseDiaryColumnTitles.SLEEP_HOURS to ChronoUnit.HOURS.between(
                        activityDayEndDateTime,
                ...
                error("This should never happen due null checks above, but is needed to make compiler happy.")
            }
        }

        val timeRangeMapping = mapOf(
            TimeUseDiaryColumnTitles.WAKE_UP_YESTERDAY to activityDayStartDateTime.toLocalTime().format(formatter),
            TimeUseDiaryColumnTitles.BED_TIME_YESTERDAY to activityDayEndDateTime.toLocalTime().format(formatter),
            TimeUseDiaryColumnTitles.DAY_TIME_HOURS to ChronoUnit.HOURS.between(
                activityDayStartDateTime,
                activityDayEndDateTime
            ),
        ) + sleepHoursMapping
        ...
        val additionalColumTitles =
            TimeUseDiaryDownloadDataType.NightTime.downloadColumnTitles -
                defaultColumnMapping.keys -
                timeRangeMapping.keys -
                if (todayWakeUpTime == null) {
                    setOf(TimeUseDiaryColumnTitles.WAKE_UP_TODAY)
                } else {
                    setOf()
                }

        val additionalColumnMapping = additionalColumTitles.associateWith { title ->
            val code = TimeUseDiaryColumnTitles.columnTitleToQuestionCodeMap[title] ?: return@associateWith setOf("UNMAPPED")
