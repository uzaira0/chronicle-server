 ...
 */
public open class StudyComplianceHazelcastTask : HazelcastFixedRateTask<StudyComplianceHazelcastTaskDependencies> {
    internal companion object {
        ...
        internal fun complianceLogSummary(
            participantViolations: Map<String, List<ComplianceViolation>>
        ): String {
            val violationCount = participantViolations.values.sumOf { violations -> violations.size }
            val participantRefs = LogSanitizer.stableFingerprints(
                participantViolations.keys,
                prefix = "participant",
            )
            return "participantCount=${participantViolations.size}, " +
                "violationCount=$violationCount, participantRefs=$participantRefs"
        }
    }
    ...
    // reason: boundary catch — a single study's notification failure must roll back and not abort the loop
    @Suppress("TooGenericExceptionCaught")
    public fun notifyNonCompliantStudies(nonCompliantStudies: Map<UUID, Map<String, List<ComplianceViolation>>>) {
        ...
        nonCompliantStudies.forEach { (studyId, participantViolations) ->
            logger.info(
                "Sending non-compliance notification for study {} ({})",
                studyId,
                complianceLogSummary(participantViolations),
            )
...
public data class StudyComplianceHazelcastTaskDependencies(
