 ...
 */
public class PostgresEventColumns private constructor() {
    internal companion object {
        private val logger = LoggerFactory.getLogger(PostgresEventColumns::class.java)

        @JvmField val APP_PACKAGE_NAME = PostgresColumnDefinition("app_package_name", PostgresDatatype.TEXT)
        @JvmField val ACTIVITY_CLASS = PostgresColumnDefinition("activity_class", PostgresDatatype.TEXT)
        ...
        // Convert all durations to milliseconds for stable Postgres storage and exports.
        @JvmField val DURATION = PostgresColumnDefinition("duration", PostgresDatatype.BIGINT )

        public val FQNS_TO_COLUMNS: Map<FullQualifiedName, PostgresColumnDefinition> = mapOf(
//                EdmConstants.OL_ID_FQN to ID,
        ...
        // Durable device queue-write time; defaults to receipt time for clients that predate it.
        @JvmField val COLLECTED_AT = PostgresColumnDefinition("collected_at", PostgresDatatype.TIMESTAMPTZ).withDefault("now()").notNull()
        ...
        @JvmField val APP_DURATION_SECONDS = PostgresColumnDefinition("app_duration_seconds", PostgresDatatype.DOUBLE)
        @JvmField val APP_RECORD_TYPE = PostgresColumnDefinition("app_record_type", PostgresDatatype.TEXT)
        ...
        @JvmField val IOS_SCREEN_TIME_SOURCE = PostgresColumnDefinition("ios_screen_time_source", PostgresDatatype.TEXT)
        @JvmField val IOS_SCREEN_TIME_CONFIDENCE = PostgresColumnDefinition("ios_screen_time_confidence", PostgresDatatype.TEXT)
        @JvmField val IOS_SCREEN_TIME_ROW_KIND = PostgresColumnDefinition("ios_screen_time_row_kind", PostgresDatatype.TEXT)
        @JvmField val IOS_SCREEN_TIME_APP_LABEL = PostgresColumnDefinition("ios_screen_time_app_label", PostgresDatatype.TEXT)
        @JvmField val IOS_SCREEN_TIME_BUNDLE_ID = PostgresColumnDefinition("ios_screen_time_bundle_id", PostgresDatatype.TEXT)
        @JvmField val IOS_SCREEN_TIME_WEB_DOMAIN = PostgresColumnDefinition("ios_screen_time_web_domain", PostgresDatatype.TEXT)
        @JvmField val IOS_SCREEN_TIME_RAW_SOURCE_LABEL = PostgresColumnDefinition("ios_screen_time_raw_source_label", PostgresDatatype.TEXT)
        @JvmField val IOS_SCREEN_TIME_NOTIFICATION_COUNT = PostgresColumnDefinition("ios_screen_time_notification_count", PostgresDatatype.INTEGER)
        @JvmField val IOS_SCREEN_TIME_PICKUP_COUNT = PostgresColumnDefinition("ios_screen_time_pickup_count", PostgresDatatype.INTEGER)

        // Reuse the existing nullable raw-text slot for versioned motion batches. SENSOR_TYPE
        // disambiguates motion payloads from Screen Time source labels without live-table DDL.
        public val RAW_SENSOR_PAYLOAD: PostgresColumnDefinition
            get() = IOS_SCREEN_TIME_RAW_SOURCE_LABEL
        ...
        public val columnTypes : Map<String, PostgresDatatype> = eventStorageColumns().associate { it.name to it.datatype }

        @JvmStatic
        public fun eventStorageColumns(): List<PostgresColumnDefinition> {
            return (PostgresEventColumns::class.java.fields.asList() + PostgresEventColumns::class.java.declaredFields)
                ...
                .mapNotNull { field: Field ->
                    try {
                        ...
                        logger.debug("Skipping inaccessible column field {}", field.name, e)
        ...
        public val SHARED_SENSOR_COLS = linkedSetOf(
            STUDY_ID,
            PARTICIPANT_ID,
        ...
        public val DEVICE_USAGE_SENSOR_COLS = linkedSetOf(
            TOTAL_SCREEN_WAKES,
            ...
            IOS_SCREEN_TIME_NOTIFICATION_COUNT,
            IOS_SCREEN_TIME_PICKUP_COUNT
        )

        public val ACCELEROMETER_SENSOR_COLS = linkedSetOf(RAW_SENSOR_PAYLOAD)

        public val PHONE_USAGE_SENSOR_COLS = linkedSetOf(
        ...
        public val MESSAGES_USAGE_SENSOR_COLS = linkedSetOf(
            TOTAL_INCOMING_MESSAGES,
            TOTAL_OUTGOING_MESSAGES,
            TOTAL_UNIQUE_CONTACTS
        )

        public val KEYBOARD_METRICS_SENSOR_COLS = linkedSetOf(
            TOTAL_WORDS,
            TOTAL_ALTERED_WORDS,
            TOTAL_TAPS,
            TOTAL_DRAGS,
        ...
        public val IOS_UTILITY_COLS = linkedSetOf(EXACT_RECORDED_DATE_TIME)
    }
}
