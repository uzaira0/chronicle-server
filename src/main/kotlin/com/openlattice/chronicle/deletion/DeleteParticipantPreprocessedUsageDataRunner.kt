...
public open class DeleteParticipantPreprocessedUsageDataRunner(
    private val storageResolver: StorageResolver
) : AbstractChronicleDeleteJobRunner<DeleteParticipantPreprocessedUsageData>() {

    internal companion object {
        private val logger = LoggerFactory.getLogger(DeleteParticipantPreprocessedUsageDataRunner::class.java)!!

        private val DELETE_PARTICIPANT_PREPROCESSED_USAGE_DATA_SQL = """
            DELETE FROM ${PREPROCESSED_USAGE_EVENTS.name}
            WHERE ${STUDY_ID.name} = ?
    ...
    override fun runJob(connection: Connection, job: ChronicleJob): List<AuditableEvent> {
        ...
        job.definition as DeleteParticipantPreprocessedUsageData

        val deletedRows = eventHds.connection.use { eventConnection ->
            deletePreprocessedUsageData(eventConnection, job.definition)
        }
        ...
        return listOf(
            AuditableEvent(
                ...
                job.principal,
                eventType = AuditEventType.BACKGROUND_PREPROCESSED_USAGE_DATA_DELETION,
    ...
    private fun deletePreprocessedUsageData(connection: Connection, jobDefinition: DeleteParticipantPreprocessedUsageData): Long {
        logger.info(
            "Deleting preprocessed usage data with studyId = {} for participantRefs = {}",
            jobDefinition.studyId,
            LogSanitizer.stableFingerprints(jobDefinition.participantIds, "participant")
        )
        return connection.prepareStatement(DELETE_PARTICIPANT_PREPROCESSED_USAGE_DATA_SQL).use { ps ->
    ...
    override fun accepts(): Class<DeleteParticipantPreprocessedUsageData> = DeleteParticipantPreprocessedUsageData::class.java
}
