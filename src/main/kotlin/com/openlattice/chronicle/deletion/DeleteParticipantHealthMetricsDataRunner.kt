...
public open class DeleteParticipantHealthMetricsDataRunner : AbstractChronicleDeleteJobRunner<DeleteParticipantHealthMetricsData>() {
    internal companion object {
        private val logger = LoggerFactory.getLogger(DeleteParticipantHealthMetricsDataRunner::class.java)!!

        private val DELETE_PARTICIPANT_HEALTH_METRICS_SQL = """
            DELETE FROM ${HEALTH_METRICS.name}
            WHERE ${STUDY_ID.name} = ?
    ...
    override fun runJob(connection: Connection, job: ChronicleJob): List<AuditableEvent> {
        job.definition as DeleteParticipantHealthMetricsData

        val deletedRows = deleteHealthMetricsData(connection, job.definition)
        ...
        return listOf(
            AuditableEvent(
                ...
                job.principal,
                eventType = AuditEventType.BACKGROUND_HEALTH_METRICS_DATA_DELETION,
    ...
    private fun deleteHealthMetricsData(connection: Connection, jobDefinition: DeleteParticipantHealthMetricsData): Long {
        logger.info(
            "Deleting health metrics data with studyId = {} for participantRefs = {}",
            jobDefinition.studyId,
            LogSanitizer.stableFingerprints(jobDefinition.participantIds, "participant")
        )
        return connection.prepareStatement(DELETE_PARTICIPANT_HEALTH_METRICS_SQL).use { ps ->
    ...
    override fun accepts(): Class<DeleteParticipantHealthMetricsData> = DeleteParticipantHealthMetricsData::class.java
}
