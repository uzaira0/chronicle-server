...
public open class HazelcastMap<K, V> internal constructor(public val name: String) : TypedMapIdentifier<K, V> {
    ...
    public fun getMap(hazelcast: HazelcastInstance): IMap<K, V> {
        this.checker.check()
...
//        @JvmField val EXTERNAL_TABLES = HazelcastMap<UUID, ExternalTable>("EXTERNAL_TABLES")
        ...
        @JvmField val JWT_BLOCKLIST = HazelcastMap<String, Long>("JWT_BLOCKLIST")
...
//      @JvmField val SMS_INFORMATION = HazelcastMap<SmsInformationKey, SmsEntitySetInformation>("SMS_INFORMATION")
        @JvmField val USERS = HazelcastMap<String, ChronicleUserProfile>("USERS")

        // @formatter:on

        @JvmStatic
        public fun values(): Array<HazelcastMap<*, *>> {
            return valuesCache.toTypedArray()
        }

        @JvmStatic
        public fun valueOf(name: String): HazelcastMap<*, *> {
