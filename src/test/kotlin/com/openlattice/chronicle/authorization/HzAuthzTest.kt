...
open class HzAuthzTest : ChronicleServerTests() {
    @Test
    fun roleAndAuthorizationServiceLockProxiesAreReentrantOnConfiguredHazelcastMap() {
        val key = AclKey(UUID.randomUUID())
        val roleServiceLocks = HazelcastMap.SECURABLE_OBJECT_TYPES.getMap(hazelcastInstance)
        val authorizationServiceLocks = HazelcastMap.SECURABLE_OBJECT_TYPES.getMap(hazelcastInstance)
        var nestedLockAcquired = false

        roleServiceLocks.lock(key)
        try {
            nestedLockAcquired = authorizationServiceLocks.tryLock(key)
            Assert.assertTrue(nestedLockAcquired)
        } finally {
            if (nestedLockAcquired) {
                authorizationServiceLocks.unlock(key)
            }
            roleServiceLocks.unlock(key)
        }
    }

    @Test
    fun expiredAceIsExcludedFromEveryAuthorizationReadAndOwnerPath() {
        val key = AclKey(UUID.randomUUID())
        val permanentOwner = initializePrincipal(TestDataFactory.userPrincipal())
        val expiredPrincipal = initializePrincipal(TestDataFactory.userPrincipal())
        val activePrincipal = initializePrincipal(TestDataFactory.userPrincipal())
        val expiredPermissions = EnumSet.of(Permission.READ, Permission.OWNER)
        val activeExpiration = OffsetDateTime.now().plusDays(1)
        ...
        Assert.assertFalse(
            hzAuthz.checkIfHasPermissions(
                key,
                setOf(expiredPrincipal),
                EnumSet.of(Permission.READ)
            )
        )
        Assert.assertFalse(
            hzAuthz.authorize(
                mapOf(key to EnumSet.of(Permission.READ)),
                setOf(expiredPrincipal)
            ).getValue(key).getValue(Permission.READ)
        )
        Assert.assertTrue(
            hzAuthz.getSecurableObjectPermissions(key, setOf(expiredPrincipal)).isEmpty()
        )
        Assert.assertTrue(
            hzAuthz.getSecurableObjectSetsPermissions(
                listOf(setOf(key)),
                setOf(expiredPrincipal)
            ).getValue(setOf(key)).isEmpty()
        )
        Assert.assertFalse(
            hzAuthz.listAuthorizedObjectsOfType(
                setOf(expiredPrincipal),
                SecurableObjectType.Study,
                EnumSet.of(Permission.READ)
            ).contains(key)
        ...
        Assert.assertFalse(
            hzAuthz.getAuthorizedObjectsOfTypes(
                setOf(expiredPrincipal),
                setOf(SecurableObjectType.Study),
                EnumSet.of(Permission.READ)
            ).toList().contains(key)
        )
        Assert.assertFalse(
            hzAuthz.getAuthorizedPrincipalsOnSecurableObject(
                key,
                EnumSet.of(Permission.READ)
            ).contains(expiredPrincipal)
        )
        Assert.assertFalse(hzAuthz.getSecurableObjectOwners(key).contains(expiredPrincipal))
        Assert.assertTrue(hzAuthz.getSecurableObjectOwners(key).contains(permanentOwner))
        Assert.assertFalse(
            hzAuthz.getOwnersForSecurableObjects(setOf(key))[key].contains(expiredPrincipal)
        )
        Assert.assertTrue(
            hzAuthz.getOwnersForSecurableObjects(setOf(key))[key].contains(permanentOwner)
        )

        val acl = hzAuthz.getAllSecurableObjectPermissions(key)
        Assert.assertFalse(acl.aces.any { it.principal == expiredPrincipal })
        Assert.assertEquals(
            activeExpiration,
            acl.aces.single { it.principal == activePrincipal }.expirationDate
        )
    }
    ...
    fun testTypeMismatchPermission() {
        ...
        hzAuthz.createUnnamedSecurableObject(key, p, EnumSet.of(Permission.OWNER), SecurableObjectType.Study)
        hzAuthz.addPermission(key, p, permissions)
    ...
    open fun testListSecurableObjects() {
        ...
        hzAuthz.createUnnamedSecurableObject(key, p2, EnumSet.of(Permission.OWNER), SecurableObjectType.Study)
    ...
    fun testAccessChecks() {
        ...
        for (i in 0 until size) {
            accessChecks.add(populateAccessCheck(i, aclKeys, p2s, permissions1s, permissions2s, all))
        }
        var i = 0
    ...
    private fun populateAccessCheck(
        i: Int,
        aclKeys: Array<AclKey?>,
        p2s: Array<Principal?>,
        permissions1s: Array<EnumSet<Permission>?>,
        permissions2s: Array<EnumSet<Permission>?>,
        all: EnumSet<Permission>,
    ): AccessCheck {
        val key: AclKey = AclKey(UUID.randomUUID())
        ...
        return AccessCheck(key, permissions1)
    }
    ...
    companion object {
        ...
        // reason: retained for its side effect — initializePrincipal registers the ADMIN
        // securable principal during test setup; the returned value itself is not read
        ...
        fun initializePrincipal(principal: Principal): Principal {
            return initializePrincipal(principal, principal)
        }

        @JvmStatic
        fun initializePrincipal(owner: Principal, principal: Principal): Principal {
            init()
            ...
            try {
                spm.createSecurablePrincipalIfNotExists(owner, sp)
            } catch (e: Exception) {
